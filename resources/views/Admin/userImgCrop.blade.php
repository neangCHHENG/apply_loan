...
<div class="modal fade" id="imageCropDiv" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button class="btn btn-info" onclick="new function () { $('#inputImage').click()}">Browse</button>
                <button class="btn btn-primary" onclick="new function () { $('#imagecrop600600').click()}">Set</button>
            </div>
        </div>
    </div>
</div>
...
<script>
                ...
                switch (data.method) {
                    ...
                    case 'getCroppedCanvas':
                        if (result) {
                            ...
                            saveUserImage(result.toDataURL('image/jpeg'));
    ...
    function saveUserImage(base64img) {
        var userid = $("#userphotoid").html();
        resizeImage(base64img, 50, 50).then(compress => {
            $.ajax({
                url: "{{ url('api/admin/saveUserImage') }}",
                type: "POST",
                data: {
                    _token: formToken,
                    base64img: base64img,
                ...
                success: function(response) {
                    ...
                    //User Datatable Need to refresh image too//
                    var table = $('#userlist').DataTable();
                    var selectRow = table.row($('#userlist #' + userid));
                    selectRow.data().photo = compress;
                    initUserList();
                    //User Datatable Need to refresh image too//

                    if (appuserid == userid) {
                        getSocialUserImage(); //Refresh image if current user change their own image
                    }
