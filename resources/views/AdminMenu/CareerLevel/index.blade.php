<div class="col-xl-12">
    <div id="panel-1" class="panel">
        <div class="panel-hdr">
            <h2>
                Career Level <span class="fw-300"><i>List</i></span>
            </h2>
...
@include('AdminMenu.CareerLevel.form')
<script>
    {
        var idEdit = null;
        ...
        function getdata() {
            $.ajax({
                url: "{{ url('api/admin/career-level') }}",
                type: "GET",
        ...
        function update() {
            if (0 === 0) {
                $.ajax({
                    url: "{{ url('/api/admin/career-level') }}/" + idEdit,
                    type: "PUT",
                    data: valueFil(),
                    beforeSend: function(xhr) {
                        blockagePage('Please Wait...');
        ...
        function destroy(id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "{{ url('/api/admin/career-level') }}/" + id,
                        type: "DELETE",
                        beforeSend: function(xhr) {
                            blockagePage('Please Wait...');
                            xhr.setRequestHeader('Authorization', 'Bearer ' + AuthToken);
